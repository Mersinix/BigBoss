          ...
          <div className={`absolute inset-x-0 top-0 z-[60] transition-transform duration-300 ease-in-out ${filterOpen ? "translate-y-0" : "-translate-y-full"}`}>
            <div className="bg-gray-900/95 backdrop-blur-xl rounded-b-3xl shadow-2xl">
              ...
              <div className="px-5 pb-5 space-y-4">
                <div>
                  <p className="text-gray-400 text-xs mb-2">Service</p>
                  <div className="flex flex-wrap gap-2 max-h-24 overflow-y-auto">
                    <button onClick={() => setPendingCategory("")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${!pendingCategory ? "bg-purple-500 text-white" : "bg-white/10 text-gray-300 hover:bg-white/15"}`}>
                      {!pendingCategory && <Check className="w-3 h-3" />} Tous
                    </button>
                    {allCategories.map((c) => (
                      <button key={c} onClick={() => setPendingCategory(c)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${pendingCategory === c ? "bg-purple-500 text-white" : "bg-white/10 text-gray-300 hover:bg-white/15"}`}>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 max-h-24 overflow-y-auto">
                    <button onClick={() => setPendingLocation("")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${!pendingLocation ? "bg-purple-500 text-white" : "bg-white/10 text-gray-300 hover:bg-white/15"}`}>
                      {!pendingLocation && <Check className="w-3 h-3" />} Toutes
                    </button>
                    {allLocations.map((l) => (
                      <button key={l} onClick={() => setPendingLocation(l)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${pendingLocation === l ? "bg-purple-500 text-white" : "bg-white/10 text-gray-300 hover:bg-white/15"}`}>
                        {pendingLocation === l && <Check className="w-3 h-3" />} {l}
                      </button>
                    ))}
                  </div>
                </div>
                <button onClick={applyFilter} className="w-full py-3 bg-purple-500 hover:bg-purple-400 text-white font-semibold rounded-2xl transition-colors active:scale-[.98]">
                  Appliquer
                </button>
              </div>
            </div>
