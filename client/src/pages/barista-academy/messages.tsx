...
// filtered to service "ACADEMY". Mirrors barista-marketplace/messages.tsx
// exactly — see that file's own note on why this is the shared system.
export default function AcademyMessagesPage() {
  ...
  const academyConversations = conversations.filter((conversation) => conversation.service === "ACADEMY");
  const activeConversation = academyConversations.find((conversation) => conversation.id === activeId) ?? null;
          ...
          {!activeConversation ? (
            <div className="max-h-[480px] overflow-y-auto">
              {isLoading ? (
                ...
                <div className="px-5 py-16 text-center">
                  ...
                  <p className="text-xs text-muted-foreground/70 mt-1">Les messages des Coffee Owners apparaîtront ici en temps réel.</p>
                </div>
              ) : (
                academyConversations.map((conversation) => {
                  const name = conversation.title ?? (conversation.otherParticipants.map((participant) => participant.name).join(", ") || "Coffee Owner");
                  return (
                    <button
                      key={conversation.id}
                      onClick={() => selectConversation(conversation.id)}
                      className="w-full flex items-center gap-3 px-5 py-3 text-left border-b border-border/50 last:border-0 hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20 transition-colors"
                    ...
                    >
                      <Avatar className="w-9 h-9 shrink-0">
                        ...
                        <AvatarFallback className="bg-indigo-100 text-indigo-700 font-bold text-xs">{name.charAt(0).toUpperCase()}</AvatarFallback>
                      </Avatar>
                      ...
                      {conversation.unreadCount > 0 && (
                        <span className="w-5 h-5 rounded-full bg-indigo-600 text-white text-[10px] font-bold flex items-center justify-center">{conversation.unreadCount}</span>
                      )}
                    </button>
                  );
                })
            ...
            <div className="h-[480px] flex flex-col">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-border/50">
                ...
                <span className="ml-auto text-[10px] font-semibold px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700">ACADEMY</span>
              </div>
                ...
                {messagesLoading ? <p className="text-xs text-muted-foreground text-center pt-6">Chargement…</p> : (messagesData?.messages ?? []).map((message) => {
                  ...
                  return (
                    <div key={message.id} className={`flex ${own ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${own ? "bg-indigo-600 text-white rounded-br-sm" : "bg-secondary rounded-bl-sm"}`}>
                        {message.content}
              ...
              <div className="p-3 border-t border-border/50 flex gap-2">
                ...
                <Button
                  ...
                  className="h-9 w-9 shrink-0 bg-indigo-600 hover:bg-indigo-700 rounded-xl"
                  aria-label="Envoyer"
