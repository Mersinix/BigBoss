...
// Role → left-border color indicator for SHOP conversations
const ROLE_INDICATOR: Record<string, { border: string; dot: string }> = {
  ADMIN:            { border: "border-l-red-500",   dot: "bg-red-500" },
  SUPER_ADMIN:      { border: "border-l-red-500",   dot: "bg-red-500" },
  SUPPLIER:         { border: "border-l-amber-500", dot: "bg-amber-500" },
  DELIVERY_COMPANY: { border: "border-l-green-500", dot: "bg-green-500" },
...
function ShopTab({ userId }: { userId: number }) {
  ...
  const { data: msgsData, isLoading: msgsLoading } = useQuery<{ messages: ConversationMessageRow[] }>({
          ...
          <div className="p-3 border-b border-border/50 shrink-0">
            <div className="relative">
              ...
              <Input
                ...
                placeholder="Search SHOP conversations…"
                value={search}
          ...
          <div className="flex-1 overflow-y-auto">
            {convsLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full py-16 text-center">
                ...
                <p className="font-medium text-gray-500">No SHOP conversations yet</p>
                <p className="text-sm text-gray-400 mt-1">Place an order to start chatting with suppliers</p>
              </div>
            ) : (
              filtered.map((conv) => {
                const otherRole = conv.otherParticipants[0]?.role ?? "";
                const style = ROLE_INDICATOR[otherRole] ?? { border: "border-l-transparent", dot: "" };
                const name = conv.otherParticipants.map(p => p.name).join(", ") || "Unknown";
                return (
                  <button
                    key={conv.id}
                    data-testid={`button-thread-${conv.id}`}
                    onClick={() => openConv(conv.id)}
                    className={`w-full flex items-start gap-3 p-3 text-left hover:bg-secondary/60 transition-colors border-b border-border/30 last:border-0 border-l-2 ${style.border}`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-1">
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          {style.dot && <span className={`shrink-0 w-1.5 h-1.5 rounded-full ${style.dot}`} />}
                        </div>
        ...
        <>
          <div className="flex items-center gap-3 p-4 border-b border-border/50 shrink-0">
            ...
            <Avatar className="w-8 h-8">
              <AvatarFallback className="text-xs bg-primary/10 text-primary font-bold">
                {(activeConv.otherParticipants[0]?.name ?? "?").charAt(0).toUpperCase()}
              </AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0 flex items-center gap-2">
              <span className="font-semibold text-sm text-foreground">
                {activeConv.otherParticipants.map(p => p.name).join(", ")}
              </span>
              <Badge className={`text-[10px] border-0 px-1.5 py-0.5 ${SERVICE_BADGE["SHOP"]}`}>SHOP</Badge>
            </div>
          </div>
            ...
            {msgsLoading ? (
              ...
              return (
                <div key={m.id} className={`flex ${isOwn ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-xs px-4 py-2.5 rounded-2xl text-sm ${isOwn ? "bg-primary text-primary-foreground rounded-br-sm" : "bg-secondary text-foreground rounded-bl-sm"}`}>
                    <p>{m.content}</p>
          ...
          <div className="p-4 border-t border-border/50 flex gap-2 shrink-0">
            <Input
              ...
              onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); if (input.trim()) sendMutation.mutate(input.trim()); } }}
              disabled={sendMutation.isPending}
            />
            <Button size="icon" data-testid="button-send-message" onClick={() => { if (input.trim()) sendMutation.mutate(input.trim()); }} disabled={!input.trim() || sendMutation.isPending}>
...
// ── Static tab (PRINT / BARISTA / MARKETING, unchanged) ──────────────────────

function StaticTab({ service }: { service: ServiceId }) {
  const [threads, setThreads] = useState(fakeThreads.filter(t => t.service === service));
  ...
  return (
    <>
      {view === "list" && (
        <>
          <div className="p-3 border-b border-border/50 shrink-0">
            <div className="relative">
              <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground" />
              <Input className="pl-8 h-8 text-sm" placeholder={`Search ${service} conversations…`} value={search} onChange={(e) => setSearch(e.target.value)} data-testid="input-search-messages" />
            </div>
          </div>
          <div className="flex-1 overflow-y-auto">
            {filtered.length === 0 ? (
              ...
              filtered.map(t => (
                <button key={t.id} data-testid={`button-thread-${t.id}`} onClick={() => openConversation(t)} className="w-full flex items-start gap-3 p-3 text-left hover:bg-secondary/60 transition-colors border-b border-border/30 last:border-0">
                  <Avatar className="w-9 h-9 shrink-0">
                    <AvatarFallback className="text-xs bg-primary/10 text-primary font-bold">{t.name.charAt(0)}</AvatarFallback>
                  </Avatar>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-1">
                  ...
                  {t.unread > 0 && (
                    <span className="shrink-0 bg-primary text-primary-foreground text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center mt-0.5">{t.unread}</span>
                  )}
                </button>
              ))
            )}
      ...
      {view === "chat" && active && (
        <>
          <div className="flex items-center gap-3 p-4 border-b border-border/50 shrink-0">
            <button onClick={() => setView("list")} data-testid="button-chat-back" className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground transition-colors">
              <ChevronLeft className="w-4 h-4" /><span className="text-xs">Back</span>
            </button>
            <Avatar className="w-8 h-8">
              <AvatarFallback className="text-xs bg-primary/10 text-primary font-bold">{active.name.charAt(0)}</AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0 flex items-center gap-2">
              <span className="font-semibold text-sm text-foreground">{active.name}</span>
              <Badge className={`text-[10px] border-0 px-1.5 py-0.5 ${SERVICE_BADGE[active.service]}`}>{active.service}</Badge>
            </div>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {active.messages.map((m, i) => (
              <div key={i} className={`flex ${m.from === "me" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-xs px-4 py-2.5 rounded-2xl text-sm ${m.from === "me" ? "bg-primary text-primary-foreground rounded-br-sm" : "bg-secondary text-foreground rounded-bl-sm"}`}>
                  <p>{m.text}</p>
          ...
          <div className="p-4 border-t border-border/50 flex gap-2 shrink-0">
            <Input data-testid="input-message" className="flex-1" placeholder="Type a message..." value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} />
            <Button size="icon" data-testid="button-send-message" onClick={send}><Send className="w-4 h-4" /></Button>
          </div>
        </>
      )}
    </>
  ...
  return (
    <div className="flex flex-col p-6 gap-5 max-w-2xl mx-auto">
      ...
      <Card className="overflow-hidden flex flex-col" style={{ height: 520 }}>
        {service === "SHOP" ? (
          <ShopTab userId={user?.id ?? 0} />
        ) : (
          <StaticTab service={service} />
        )}
      </Card>
    </div>
  );
