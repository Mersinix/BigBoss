...
export default function ProductDetailPage() {
  ...
  const { data: product, isLoading, error } = useQuery<MarketplaceProduct>({
    queryKey: ["/api/marketplace", params.productId],
    queryFn: async () => {
      const res = await fetch(`/api/marketplace/${params.productId}`);
  ...
  if (error || !product) {
    return (
      <div className="p-6 text-center py-32">
        ...
        <Button variant="outline" className="mt-4" onClick={() => navigate("/products")}>Back to Browse</Button>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <button onClick={() => navigate("/products")} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors" data-testid="button-back-browse">
