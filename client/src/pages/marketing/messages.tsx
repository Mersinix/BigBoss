...
// and pages/printer/messages.tsx. No separate/isolated chat system. ───────────
...
export default function MarketingMessages() {
  ...
  const marketingConversations = conversations.filter((c) => c.service === "MARKETING");
  const activeConversation = marketingConversations.find((c) => c.id === activeId) ?? null;
  ...
  const sendMessage = useMutation({
    mutationFn: (content: string) => apiRequest("POST", `/api/messages/conversations/${activeId}/messages`, { content }),
    onSuccess: () => {
      setInput("");
          ...
          <div className="max-h-[420px] overflow-y-auto">
            {isLoading ? (
              ...
              <div className="px-5 py-10 text-center">
                <MessageCircle className="w-9 h-9 text-muted-foreground/30 mx-auto mb-2" />
                <p className="text-sm font-medium text-muted-foreground">Aucun message pour le moment</p>
                <p className="text-xs text-muted-foreground/70 mt-1">Les demandes des Coffee Owners apparaîtront ici.</p>
              </div>
            ) : (
              marketingConversations.map((conversation) => {
                const name = conversation.title ?? (conversation.otherParticipants.map((p) => p.name).join(", ") || "Coffee Owner");
                return (
                  <button
                    key={conversation.id}
                    onClick={() => selectConversation(conversation.id)}
                    className="w-full flex items-center gap-3 px-5 py-3 text-left border-b border-border/50 last:border-0 hover:bg-fuchsia-50/50 dark:hover:bg-fuchsia-500/5 transition-colors"
                  >
                    <Avatar className="w-9 h-9 shrink-0">
                      <AvatarImage src={getAvatarUrl(conversation.otherParticipants[0] as any)} alt={name} />
                      <AvatarFallback className="bg-fuchsia-100 text-fuchsia-700 font-bold text-xs">{name.charAt(0).toUpperCase()}</AvatarFallback>
                    </Avatar>
                    ...
                    {conversation.unreadCount > 0 && (
                      <span className="w-5 h-5 rounded-full bg-fuchsia-600 text-white text-[10px] font-bold flex items-center justify-center">{conversation.unreadCount}</span>
                    )}
                  </button>
                );
              })
          ...
          <div className="h-[420px] flex flex-col">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-border/50">
              ...
              <p className="text-sm font-semibold truncate">
                {activeConversation.title ?? (activeConversation.otherParticipants.map((p) => p.name).join(", ") || "Coffee Owner")}
              </p>
              <span className="ml-auto text-[10px] font-semibold px-1.5 py-0.5 rounded bg-fuchsia-100 text-fuchsia-700">MARKETING</span>
            </div>
              ...
              {messagesLoading ? <p className="text-xs text-muted-foreground text-center pt-6">Chargement…</p> : (messagesData?.messages ?? []).map((message) => {
                ...
                return (
                  <div key={message.id} className={`flex ${own ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${own ? "bg-fuchsia-600 text-white rounded-br-sm" : "bg-secondary text-foreground rounded-bl-sm"}`}>
                      {message.content}
            ...
            <div className="p-3 border-t border-border/50 flex gap-2">
              ...
              <Button
                ...
                className="h-9 w-9 shrink-0 bg-fuchsia-600 hover:bg-fuchsia-700 rounded-xl"
                aria-label="Envoyer"
              >
