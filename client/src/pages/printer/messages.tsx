...
// (Shop, Maintenance, Barista Marketplace) — same tables, same endpoints,
// filtered to service "PRINT". Mirrors barista-marketplace/messages.tsx
// field-for-field (the closest analog: another independent service-provider
// role messaging Coffee Owners) so Coffee Owner <-> Printer conversations
// stay in the one shared messaging system — no separate PRINT messaging
// architecture.
export default function PrinterMessagesPage() {
  ...
  const printConversations = conversations.filter((conversation) => conversation.service === "PRINT");
  const activeConversation = printConversations.find((conversation) => conversation.id === activeId) ?? null;
  ...
  return (
    <div className="flex flex-col gap-5">
      <DashboardHero
        title="Messages"
        subtitle="Vos conversations avec les cafés."
        icon={MessageCircle}
      ...
      <Card className="bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700/60 rounded-2xl overflow-hidden">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-semibold flex items-center gap-2">
            <MessageCircle className="w-4 h-4 text-blue-600" />Conversations
          </CardTitle>
        </CardHeader>
          ...
          {!activeConversation ? (
            <div className="max-h-[480px] overflow-y-auto">
              {isLoading ? (
                ...
                printConversations.map((conversation) => {
                  const name = conversation.title ?? (conversation.otherParticipants.map((participant) => participant.name).join(", ") || "Café");
                  return (
                    <button
                      key={conversation.id}
                      onClick={() => selectConversation(conversation.id)}
                      className="w-full flex items-center gap-3 px-5 py-3 text-left border-b border-border/50 last:border-0 hover:bg-blue-50/50 dark:hover:bg-blue-950/20 transition-colors"
                    ...
                    >
                      <Avatar className="w-9 h-9 shrink-0">
                        ...
                        <AvatarFallback className="bg-blue-100 text-blue-700 font-bold text-xs">{name.charAt(0).toUpperCase()}</AvatarFallback>
                      </Avatar>
                      ...
                      {conversation.unreadCount > 0 && (
                        <span className="w-5 h-5 rounded-full bg-blue-600 text-white text-[10px] font-bold flex items-center justify-center">{conversation.unreadCount}</span>
                      )}
                    </button>
                  );
                })
            ...
            <div className="h-[480px] flex flex-col">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-border/50">
                ...
                <span className="ml-auto text-[10px] font-semibold px-1.5 py-0.5 rounded bg-blue-100 text-blue-700">PRINT</span>
              </div>
                ...
                {messagesLoading ? <p className="text-xs text-muted-foreground text-center pt-6">Chargement…</p> : (messagesData?.messages ?? []).map((message) => {
                  ...
                  return (
                    <div key={message.id} className={`flex ${own ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${own ? "bg-blue-600 text-white rounded-br-sm" : "bg-secondary rounded-bl-sm"}`}>
                        {message.content}
              ...
              <div className="p-3 border-t border-border/50 flex gap-2">
                ...
                <Button
                  ...
                  className="h-9 w-9 shrink-0 bg-blue-600 hover:bg-blue-700 rounded-xl"
                  aria-label="Envoyer"
